...
G4VPhysicalVolume* DetectorConstruction::ConstructWorld()
{
    ...
    for (double x = -4.; x < 4.; x += 1.){
        for (double y = -4.; y < 4.; y += 1.){
            ...
            backplane_pos = G4ThreeVector((x+0.5)*CLHEP::cm, (y+0.5)*CLHEP::cm,  -7.5*CLHEP::cm);
    ...
    for (double y = -4.; y < 4.; y += 1.){
        ...
        //G4cout << x+4 << "\t" << y+4  << "\t" << mask_array[int(y)+4][int(x)+4] << "\n";
        if(mask_array[int(y)+4][int(x)+4] == 1){
            ...
            mask_pos = G4ThreeVector((x+0.5)*CLHEP::cm, (y+0.5)*CLHEP::cm, 0.0*CLHEP::m);
